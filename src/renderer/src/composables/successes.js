    ...
    function calculateSuccesses(teamName, sessions) {
            ...
            if(session.team.id === id_of_team){
                ...
                if(session.was_cool !== undefined && session.was_cool === false){
                    if (bad_session.has(session.game.id)) {
                        bad_session.set(session.game.id, bad_session.get(session.game.id)+ 1);
                    } else {
                ...
                if(session.duration >= 60 * 10) {
                    ...
                    enduring.value.description = i18n.t("Successes.descriptions.enduring");
                }
                if(session.duration >= 60 * 12) {
                    inexhaustible.value.unlocked = true;
                    inexhaustible.value.image = inexhaustible_logo;
                    inexhaustible.value.description = i18n.t("Successes.descriptions.inexhaustible");
                }
        ...
        for (let [key, value] of bad_session) {
            if(value >= 10){
                ...
                relentless.value.description = i18n.t("Successes.descriptions.relentless");
            }
        }
        ...
        if(session_number >= 50){
            ...
            young_gamer.value.description = i18n.t("Successes.descriptions.young_gamer");
        }
        ...
        if(session_number >= 200){
            ...
            passionnate.value.description = i18n.t("Successes.descriptions.passionnate");
        }
        ...
        if(different_games.size >= 15){
            ...
            prospector.value.description = i18n.t("Successes.descriptions.prospector");
        }
        ...
        if(session_number >= 10){
            ...
            if(percentage >= 40){
                ...
                depressed.value.description = i18n.t("Successes.descriptions.depressed");
            }
        }
