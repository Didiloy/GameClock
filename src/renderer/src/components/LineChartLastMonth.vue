<template>
  ...
  <div class="container" v-else>
    ...
    <Dialog
      ...
      modal
      dismissableMask
      :header="i18n.t('LineChartLastMonth.title')"
...
function init() {
  const _sessions = props.sessions.map((item) => ({
  ...
  const _teams = [];
  let team_names = props.teamName.split(",");
  if (team_names.length > 1) {
    for (const team of team_names) {
      _teams.push({
        id: getIdsOfTeam(team, teams.value)[0],
        name: team,
      });
    }
  } else {
    _teams.push({
      id: getIdsOfTeam(props.teamName, teams.value)[0],
      name: props.teamName,
    });
  }

  window.electron.ipcRenderer.send("linechartlastmonth", {
    sessions: _sessions,
    teams: _teams,
  });
}

window.electron.ipcRenderer.on("result_linechartlastmonth", (event, data) => {
